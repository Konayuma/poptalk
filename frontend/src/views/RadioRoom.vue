...
const callsign = ref(user.value?.callsign || readStoredString(STORAGE_KEYS.callsign, 'ROOKIE-7'))
...
watch(user, (nextUser) => {
  if (!nextUser) void router.replace({ name: 'login' })
})
...
<template>
  ...
  >
    ...
    <header class="topbar">
      <RouterLink class="brand" to="/" aria-label="Pop Talk home">
      ...
      <div class="topbar__actions">
        ...
        <span class="callsign-badge">
          <small>{{ user?.email || 'Callsign' }}</small>
          {{ displayCallsign }}
        </span>
        ...
        <button
          class="icon-button icon-button--logout"
          type="button"
          :disabled="signingOut"
          aria-label="Sign out"
          @click="signOut"
